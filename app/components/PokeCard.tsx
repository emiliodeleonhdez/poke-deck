...
const PokeCard: React.FC<PokeCardProps> = ({ pokeInfo, action }) => {
  ...
  return (
    <div
      onClick={action}
      key={pokemonId}
      className="h-full border-2 border-transparent hover:border-pokemon_strong_blue cursor-pointer bg-pokemon bg-opacity-70 hover:bg-opacity-60 w-52 h-60 m-3 rounded-md p-4 flex flex-col items-center justify-start"
    >
